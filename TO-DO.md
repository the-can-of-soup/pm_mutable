...
- [ ] Finish `MArrayType` handlers, constructors, and destructors
  ...
  - [ ] `toReporterContent` (should behave like jwklong Arrays do when DogeisCut Objects is loaded, but with ID above/below in code style, and text `MArray` with purple `M` at the bottom)
  - [ ] `toMonitorContent`
  - [ ] `toListItem`
  - [ ] `toString`
  - [ ] `toJSON`
...
- [ ] Blocks:
  ...
  - [ ] Map in-place (main logic should be in `MArrayType.prototype.mapInPlace`; should use `this.forEach((value, index) => { /* ... */ })` loop and mutate the array at the current index with the result of the lambda being called with a Mutable Array containing: value, index, array being mapped; this Mutable Array should be created once at the start of the loop and be mutated automatically after every loop; this will only work once `MArrayType.prototype.forEach` has been implemented)
  ...
  - [ ] Mapped (main logic should be in `MArrayType.prototype.map`; should create a new array, use `this.forEach((value, index) => { /* ... */ })` loop to append the result of the lambda being called with a Mutable Array containing: \[value, index, array being mapped\] to the new array; this Mutable Array should be created once at the start of the loop and be mutated automatically after every loop; this will only work once `MArrayType.prototype.forEach` has been implemented)
  - [ ] All sliced blocks (sliced from, sliced to, sliced from to, sliced from to with step; blocks themselves should have a failsafe to return an empty array if step is `0`; main logic should be in `MArrayType.prototype.slice` with args `(from = 0, to = -1, step = 1)`; should behave similarly to Python's array slicing feature, but inclusive instead of exclusive; should use a `for (let i = from; i <= to; i += step) { /* ... */ }` loop to loop over the array and append each item to a new array that is then returned)
  - [ ]  All sorted blocks (sorted asc/desc, sorted asc/desc with key, sorted asc/desc with custom comparator; default key is `value => value`, default comparator is `Scratch.Cast.compare`; should pass the key lambda the value to key, and the comparator lambda a new Mutable Array on every call containing the two values after keying to compare; main logic should be in `MArrayType.prototype.toSorted`; behavior if array being sorted is mutated by a lambda is TBD)
  ...
  - [ ] Any (main logic should be in `MArrayType.prototype.some`; equivalent of JS `Array.prototype.some` that uses a `this.forEach((value, index) => { /* ... */ })` loop; block should use `MARRAY.some` to call the lambda on each loop with a 3-element Mutable Array containing in order: value, index, array being checked; this Mutable Array should be created once at the start of the loop and be mutated automatically after every loop; this will only work once `MArrayType.prototype.forEach` has been implemented)
  - [ ] All (main logic should be in `MArrayType.prototype.every`; equivalent of JS `Array.prototype.every` that uses a `this.forEach((value, index) => { /* ... */ })` loop; block should use `MARRAY.some` to call the lambda on each loop with a 3-element Mutable Array containing in order: value, index, array being checked; this Mutable Array should be created once at the start of the loop and be mutated automatically after every loop; this will only work once `MArrayType.prototype.forEach` has been implemented)
